                ...
                .execute(object: StringCallback() {
                    override fun onSuccess(response: Response<String>?) {
                        ...
                        if (data.has("code") && data.getString("code") == "1") {
                            ComFun.showToast(this@ClientFollowActivity, "上传成功", Toast.LENGTH_SHORT)
                            ...
                            ComFun.showToast(this@ClientFollowActivity, "上传失败", Toast.LENGTH_LONG)
                        }
                    }

                    override fun uploadProgress(progress: Progress?) {
                        if (uploadDialog == null || (uploadDialog != null && !uploadDialog!!.isShowing)) {
                            uploadDialog = AlertDialog.Builder(this@ClientFollowActivity, R.style.MyDialogStyle).setCancelable(false).create()
                            uploadDialog!!.show()
                            val win: Window = uploadDialog!!.window
                            val uploadView: View = this@ClientFollowActivity.layoutInflater.inflate(R.layout.upload_dialog, null)
